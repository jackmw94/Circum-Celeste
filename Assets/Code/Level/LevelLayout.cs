...
public class LevelLayout : ScriptableObject
{
    ...
    // {
    //     int seed = Random.Range(0, SeedMaxValue);
    //     
    //     for (int i = 0; i < _cells.Length; i++)
    //     {
    //         int cellIndex = _cells.GetNextIndex(i, seed);
    //         bool cellState = _cells[cellIndex];
    //         if (!cellState)
    //         {
    //             return CellIndexToCellCoordinate(cellIndex);
    //         }
    //     }
    //
    //     throw new UnexpectedValuesException($"Could not find an empty cell in level layout {name}");
    // }
